...
@tailrec
def lex(r: ARexp, s: List[Char], counter: Int, c: Int) : Bits = s match {
    case Nil => if(nullable(r)) mkeps(r) else throw new Exception("Not matched")
    case c::cs => if(counter % c == 0) lex(simp(der(r, c)), cs, counter + 1, c)
                    else lex((der(r, c)), cs, counter + 1, c)
}           
...
def lexer(r: Rexp, s: String) : Bits = lex((internalise(erase(normalize(internalise(r))))), s.toList, 0, 10)
...
normalize(internalise(evil1(5)))

val test = ("a"%(1, 2))%(2, 4)

transform(internalise(test))